...
//         fetch('http://localhost:8000/api/v1/jobPosting/getJobs')
//             .then(response => {
//                 if (!response.ok) {
...
//             })
//             .then(data => {
//                 setJobPostings(data.data); // Assuming your ApiResponse structure has data in the 'data' field
//             })
//             .catch(error => {
...
function Jobs() {
  ...
  // Calculate the filtered jobs
  const filteredJobs = jobPostings.filter((job) => {
    const matchesSearchTerm = job.job_title?.toLowerCase().includes(searchTerm?.toLowerCase());
    const matchesStatus = selectedStatus === "All" || job.job_status === selectedStatus;
    ...
    const matchesPayRange =
      ...
      (selectedPayRange === "Below 500" && job.pay_rate < 500) ||
      (selectedPayRange === "500-1000" && job.pay_rate >= 500 && job.pay_rate <= 1000) ||
      (selectedPayRange === "Above 1000" && job.pay_rate > 1000);
    const matchesSkills = selectedSkills === "All" || job.jobSkillsRequired?.includes(selectedSkills);
            ...
            <div className="w-full">
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <option value="Adarsh Public School, Vita">Adarsh Public School, Vita</option>
                  {/* Add more companies as needed */}
                </select>
                <select
                  className="border p-2 rounded-lg w-full shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500"
          ...
          <div className="w-4/5 flex flex-col">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 grid-cols-[repeat(auto-fill,minmax(300px,1fr))]">
              {currentJobs.map((job, index) => (
                <JobEntry
                  key={index}
                  jobDate={job.last_date_to_apply.substring(0,10) || ''} // You may want to make this dynamic based on your data
                  jobTitle={job.job_title || ''}
                  jobDescription={job.job_description.substring(0, 50) + '...' || ''}
                  jobStatus={job.job_status || ''}
                  jobSkillsRequired={job.job_skills_required.slice(0,3) || []}
                  jobPay={job.pay_rate || "500"}
                  jobLocation={job.location || "Mumbai, Maharashtra"}
                  jobCompany={job.company_name || "Hexagon"}
                  jobLogo={test || null} // Assuming 'test' is the path to the logo image
                  color={cardColors[index % cardColors.length]}
                  jobData={job}
                />
              ))}
